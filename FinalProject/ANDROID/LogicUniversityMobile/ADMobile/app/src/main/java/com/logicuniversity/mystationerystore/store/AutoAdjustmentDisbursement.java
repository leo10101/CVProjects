...
import com.logicuniversity.mystationerystore.adapters.AutoCollectDetailsAdapter;
...
public class AutoAdjustmentDisbursement extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_auto_adjustment_disbursement);
        ...
        new AsyncTask<Void,Void,List<RequestDetails>>(){
            ...
            protected void onPostExecute(List<RequestDetails> res){
                ArrayAdapter<RequestDetails> adapter = new AutoCollectDetailsAdapter(getApplicationContext(),R.layout.collectrowauto,res);
    ...
    public void click(View view) {
        ...
        ArrayList<String> SuuplierCode = new ArrayList<String>();
        ...
        for (int i = 0; i < listView.getCount(); i++) {
            ...
            SupplierCodes=CodePrice.SupplierCodesAndPrice(t3.getText().toString());
            for(int j=0;j<3;j++)
            {
                if (SupplierCodes.get(j).get("SuppCode")==s1.getSelectedItem().toString())
                    Price.add(SupplierCodes.get(j).get("Price"));
            }
        ...
        for (int i = 0; i < listView.getCount(); i++)
        {
            ...
            ERequest.creatAdjustmentDetail(AVnumber,ItemCode.get(i),urlDesc,differenceS,Price.get(i),"Disbursement%20returned");
        }
    }
}
